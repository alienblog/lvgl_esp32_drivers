...
void *disp_driver_init(lv_disp_drv_t *disp_drv)
{
  ...
  // this handle was not passed back to caller, and not used anywhere, how is
  // the backlight gets controlled by app code?
#if 0  //(defined(CONFIG_LV_DISP_BACKLIGHT_SWITCH) ||
...
void disp_driver_set_px(lv_disp_drv_t *disp_drv, uint8_t *buf, lv_coord_t buf_w,
